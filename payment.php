...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        foreach ($items as $item) {
            $stockStmt = db()->prepare('SELECT quantity FROM inventory WHERE product_id = :product_id FOR UPDATE');
        ...
        $orderStmt = db()->prepare(
            'INSERT INTO orders (customer_id, reference_number, shipping_address, total_amount, payment_method, order_status, payment_status, created_at)
             VALUES (:customer_id, :reference_number, :shipping_address, :total_amount, :payment_method, "pending", "unpaid", NOW())'
        );
        $orderStmt->execute([
        ...
        $paymentStmt = db()->prepare(
            'INSERT INTO payments (order_id, method, amount, status, proof_reference, created_at)
             VALUES (:order_id, :method, :amount, "pending", :proof_reference, NOW())'
        );
        $paymentStmt->execute([
            'order_id' => $orderId,
            'method' => $method,
